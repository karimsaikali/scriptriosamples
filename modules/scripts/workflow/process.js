...
function Process(dto) {
  ...
  if (!dto.processDefinitionName && !dto.id) {
    ...
    throw {
      	...
      	"errorCode": "Invalid_Parameter",
        "errorDetail": "Process.constructor. You need provide either a process id or a process definition name in the dto parameter"
    }
  }
  ...
  if (dto.id) {
    ...
    this._create(dto);
  	this.events = dto.events ? dto.events : this.events;
  }
...
Process.prototype.run = function() {
  ...
  for (var i = 0; i < activitiesToRun.length; i++) {
    ...
    if (activitiesToRun[i].status == common.NOT_RUNNING && activitiesToRun[i].activityDefinition.startConditionMet(this.events, this.id)) {    	      
...
Process.prototype._create = function(initDTO) {
  ...
  storage.global.workflow[this.id] = {"id": this.id}; // intialize a storage for this process instance
...
Process.prototype._load = function(id) {
  ...
  if (!processData) {
    ...
    throw {
      ...
      "errorDetail": "Process._load. Could not retrieve process instance " +  dto.processId
    }
  }
